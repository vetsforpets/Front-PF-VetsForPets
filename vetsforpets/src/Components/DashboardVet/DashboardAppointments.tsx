            ...
            <div className="space-y-10">
                {appointments.length > 0 ? (
                    appointments.map((appointment) => (
                        <div key={appointment.id} className="flex items-center gap-4 border p-4 rounded-lg shadow-sm">
                            <input type="checkbox" className="rounded-full w-5 h-5" />
                            <div>
                                <p className="text-lg font-semibold">{appointment.time}</p>
                                <p className="text-gray-600">{appointment.userName} - {appointment.petName}</p>
                            </div>
                        </div>
                    ))
                ) : (
