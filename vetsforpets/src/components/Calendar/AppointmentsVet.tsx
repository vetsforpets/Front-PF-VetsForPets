...
function AppointmentsVet() {
    ...
    const veterinarias: IVeterinaria[] = useMemo(() => [
        ...
        },
        {
            ...
            calendlyUserUri: "https://api.calendly.com/users/f41e5086-afe3-4db1-84a1-787a3a82139b",
                ...
                const events: IAppointment[] = await Promise.all(
                    data.collection.map(async (event) => {
                        ...
                        if (!inviteesResponse.ok) {
                            throw new Error(`Error ${inviteesResponse.status}: ${inviteesResponse.statusText}`);
                        }

                        const inviteesData: { collection: ICalendlyInvitee[] } = await inviteesResponse.json();
